...
''' </summary>
''' <remarks>Ojo: NO se importa <c>FO4_Base_Library.Canon</c> entero, sin calificar. Ese namespace
''' trae módulos DSL del generador de esquema (<c>WbDsl</c>) con funciones cortas como <c>Fid(...)</c>
''' que VB promueve a global — "Fid" colisiona (case-insensitive) con variables de loop tan
''' comunes como <c>fid</c> en este mismo archivo. Se referencia como <c>Canon.IOmod</c> etc. El
''' import de arriba es puntual (el módulo <c>CanonInterpretacion</c>, no todo <c>Canon</c>): trae
''' los extension methods de lectura/escritura de records (<c>ReadAttachParentSlots</c> etc.) sin traer
''' <c>WbDsl</c>.</remarks>
Public Module ObjectTemplateResolver
    ...
    Public Class CombinationResolution
        Public AppliedCombinations As New List(Of Canon.IBloque_Combinations)
        Public DirectProperties As New List(Of Canon.IBloque_Properties4)
        Public IncludedOmods As New List(Of Canon.IOmod)
    ...
    ''' </param>
    ''' <summary>Object Template (OBTS) sólo existe en el ARMO de Fallout 4 — el ARMO de Skyrim no
    ''' declara ese subrecord, así que <paramref name="armo"/> tiene que venir ya acotado a esa rama.</summary>
    Public Function ResolveArmoCombinations(armo As Canon.ArmoFO4,
        ...
        For Each fid In armo.ReadAttachParentSlots()
    ...
    Public Function ResolveNpcCombinations(combinaciones As IEnumerable(Of Canon.IBloque_Combinations),
                                           ...
                                           Optional rngSeed As Integer? = Nothing) As CombinationResolution
        Dim result As New CombinationResolution()
        If combinaciones Is Nothing Then Return result

        Dim flat As New List(Of Canon.IBloque_Combinations)
        For Each c In combinaciones
            If c IsNot Nothing Then flat.Add(c)
        Next
        If flat.Count = 0 Then Return result

        Dim initialPool As New HashSet(Of UInteger)
        If attachParentSlots IsNot Nothing Then
            For Each fid In attachParentSlots
                If fid <> 0UI Then initialPool.Add(fid)
            Next
        End If

        If raceFormID <> 0UI AndAlso pm IsNot Nothing Then
            Dim raceRec = pm.GetRecord(raceFormID)
            If raceRec IsNot Nothing AndAlso raceRec.Header.Signature = "RACE" Then
                Dim raceFo4 = TryCast(Canon.CanonRecords.Race(raceRec, pm), Canon.RaceFO4)
                If raceFo4 IsNot Nothing Then
                    For Each a In raceFo4.AttachParentSlots
                        If a.Keyword <> 0UI Then initialPool.Add(a.Keyword)
                    Next
                End If
            End If
        End If
    ...
    Private Sub ResolveCombinationList(combos As IReadOnlyList(Of Canon.IBloque_Combinations),
                                       ctxKeywords As List(Of UInteger),
                                       pm As PluginManager,
        ...
        Dim applicable As New List(Of Canon.IBloque_Combinations)
        ...
        For Each combo In combos
            ...
            Dim isApplicable As Boolean = combo.ObjectModTemplateItemDefault
            Dim reason As String = If(isApplicable, "Default", "")
            If Not isApplicable AndAlso combo.Keywords IsNot Nothing AndAlso combo.Keywords.Count > 0 AndAlso ctxKeywordSet IsNot Nothing Then
                For Each kwe In combo.Keywords
                    Dim kw = kwe.Keyword
            ...
            If logEnabled Then
                Dim kwsStr = If(combo.Keywords Is Nothing OrElse combo.Keywords.Count = 0,
                                "[]",
                                "[" & String.Join(",", combo.Keywords.Select(Function(k) "0x" & k.Keyword.ToString("X8") & "(" & KywdEditorIdSafe(k.Keyword, pm) & ")")) & "]")
                ...
                Logger.LogLazy(Function() $"[OBTE-COMBO] idx={curIdx} isDefault={combo.ObjectModTemplateItemDefault} kw={kwsStr} inc={incCount} props={propCount} applicable={isApplicable} reason={reasonLog}")
            End If
        ...
        Dim candidates As New List(Of (Omod As Canon.IOmod, ApIdx As Byte, InstanceOrdinal As Integer))
        Dim unslottedOmods As New List(Of (Omod As Canon.IOmod, ApIdx As Byte, InstanceOrdinal As Integer))
        ...
        For Each combo In applicable
            ...
            For Each inc In combo.Includes
                If inc Is Nothing OrElse inc.IncludeMod = 0UI Then Continue For
                If logEnabled Then
                    Dim incLocal = inc
                    Logger.LogLazy(Function() $"[OBTE-INC] modFid=0x{incLocal.IncludeMod:X8} apIdx={incLocal.IncludeAttachPointIndex} dontUseAll={incLocal.IncludeDonTUseAll}")
                End If
                CollectOmodCandidate(inc.IncludeMod, inc.IncludeAttachPointIndex, inc.IncludeDonTUseAll, pm, pathStack, candidates, unslottedOmods, instanceOrdinalCounter, rngSeed)
            Next
        Next
        ...
        Dim accepted As New List(Of (Omod As Canon.IOmod, ApIdx As Byte, InstanceOrdinal As Integer, HostInstanceOrdinal As Integer, HostFormID As UInteger, HostApIdx As Byte))
        ...
        Dim pending As New List(Of (Omod As Canon.IOmod, ApIdx As Byte, InstanceOrdinal As Integer))(candidates)
        Dim iterations As Integer = 0
        Const maxIter As Integer = 16
        Do
            iterations += 1
            Dim changed As Boolean = False
            Dim stillPending As New List(Of (Omod As Canon.IOmod, ApIdx As Byte, InstanceOrdinal As Integer))

            For Each cand In pending
                If Not apPool.Contains(cand.Omod.DataAttachPoint) Then
                ...
                If apProvider.TryGetValue(cand.Omod.DataAttachPoint, hostList) AndAlso hostList.Count > 0 Then
                    ...
                    Else
                        ...
                        If logEnabled Then
                            Dim apFidL = cand.Omod.DataAttachPoint
                ...
                For Each slot In cand.Omod.AttachParentSlots
                    Dim fid = slot.Keyword
                ...
                If logEnabled Then
                    Dim addedApStr = If(cand.Omod.AttachParentSlots.Count = 0,
                                        "(none)",
                                        String.Join(",", cand.Omod.AttachParentSlots.Select(
                                            Function(s) "0x" & s.Keyword.ToString("X8") & "(" & KywdEditorIdSafe(s.Keyword, pm) & ")")))
                    Logger.LogLazy(Function() $"[OBTE-POOL-ACCEPT] omod={cand.Omod.EditorID}(0x{cand.Omod.FormID:X8}) ord={cand.InstanceOrdinal} ap=0x{cand.Omod.DataAttachPoint:X8}({KywdEditorIdSafe(cand.Omod.DataAttachPoint, pm)}) host=(ord={hostOrd},0x{hostFid:X8},apIdx={hostApIdxResolved}) addedAPs=[{addedApStr}]")
                End If
            Next
        ...
        If logEnabled Then
            For Each rej In pending
                Logger.LogLazy(Function() $"[OBTE-POOL-REJECT] omod={rej.Omod.EditorID}(0x{rej.Omod.FormID:X8}) ftype={OmodFormTypeSignature(rej.Omod.DataFormType)} ap=0x{rej.Omod.DataAttachPoint:X8}({KywdEditorIdSafe(rej.Omod.DataAttachPoint, pm)}) (not in pool)")
            Next
        ...
        Dim dedupedAccepted As New List(Of (Omod As Canon.IOmod, ApIdx As Byte, InstanceOrdinal As Integer, HostInstanceOrdinal As Integer, HostFormID As UInteger, HostApIdx As Byte))
        Dim slotSeen As New Dictionary(Of (UInteger, Byte, Integer), Integer)
        For Each entry In accepted
            Dim key = (entry.Omod.DataAttachPoint, entry.ApIdx, entry.HostInstanceOrdinal)
    ...
    Private Sub CollectOmodCandidate(omodFid As UInteger,
                                     ...
                                     candidates As List(Of (Omod As Canon.IOmod, ApIdx As Byte, InstanceOrdinal As Integer)),
                                     unslotted As List(Of (Omod As Canon.IOmod, ApIdx As Byte, InstanceOrdinal As Integer)),
        ...
        Try
            ...
            Dim omod = Canon.CanonRecords.Omod(rec, pm)
            If omod Is Nothing Then Return ' record malformado para el esquema: se saltea, no se adivina.

            If omod.DataAttachPoint <> 0UI Then
                instanceOrdinalCounter += 1
                Dim ord = instanceOrdinalCounter
                candidates.Add((omod, apIdx, ord))
                If logEnabled Then
                    Dim parentSlotsStr = If(omod.AttachParentSlots.Count = 0,
                                            "(none)",
                                            String.Join(",", omod.AttachParentSlots.Select(
                                                Function(s) "0x" & s.Keyword.ToString("X8") & "(" & KywdEditorIdSafe(s.Keyword, pm) & ")")))
                    Logger.LogLazy(Function() $"[OBTE-CAND] omod={omod.EditorID}(0x{omodFid:X8}) ord={ord} ftype={OmodFormTypeSignature(omod.DataFormType)} ap=0x{omod.DataAttachPoint:X8}({KywdEditorIdSafe(omod.DataAttachPoint, pm)}) apIdx={apIdx} model='{omod.ModelFileName}' parentSlots=[{parentSlotsStr}]")
                End If
    ...
    Private Sub RecurseContainerIncludes(omod As Canon.IOmod,
                                         apIdx As Byte,
                                         dontUseAll As Boolean,
                                         pm As PluginManager,
                                         pathStack As HashSet(Of (UInteger, Byte)),
        ...
        Dim validIncludes As New List(Of Canon.IOmod_Includes)
        ...
        If dontUseAll Then
            ...
            If logEnabled Then
                ...
                Logger.LogLazy(Function() $"[OBTE-DONTUSEALL-PICK] parent={omod.EditorID}(0x{omod.FormID:X8}) mode={modeL} picks idx={idxL} modFid=0x{pickL.IncludeMod:X8} (of {validIncludes.Count})")
            End If
            CollectOmodCandidate(pick.IncludeMod, 0, pick.IncludeDonTUseAll, pm, pathStack, candidates, unslotted, instanceOrdinalCounter, rngSeed)
            Return
        End If

        For Each inc In validIncludes
            CollectOmodCandidate(inc.IncludeMod, 0, inc.IncludeDonTUseAll, pm, pathStack, candidates, unslotted, instanceOrdinalCounter, rngSeed)
        Next
    End Sub
    ...
    Private Function SelectDontUseAllIndex(omod As Canon.IOmod,
                                           apIdx As Byte,
                                           count As Integer,
