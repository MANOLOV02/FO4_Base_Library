...
''' <summary>Preflight COMPARTIDO (NPC Manager, Item_Sorter): una sola sede para elegir el juego, las rutas
''' (exe, Plugins.txt, INIs) y los plugins a cargar, validar masters, cargar y montar el diccionario de Data.
''' Lo propio de cada app entra por las propiedades de configuracion y los hooks Protected Overridable
''' (LoadSavedSelection, SaveSelection, SavePathsConfig, OnGameCommitted, RunPostLoadStepsAsync); los defaults
''' son neutros. Mudado de FO4_NPC_Manager\Preflight_Form.vb (que queda como subclase) SIN cambiar su conducta.
'''
''' Preflight dialog: el usuario apunta a Fallout4.exe y elige qué plugins cargar
...
Partial Public Class LoadOrderPreflight_Form
    ...
    ''' <summary>Titulo de la ventana (la app le pone su nombre y version).</summary>
    Public Property WindowTitle As String = "Setup"

    ''' <summary>Juegos que la app admite. Nothing = los dos. Con UN solo juego el selector queda fijo en el y la
    ''' auto-deteccion por nombre de exe no lo mueve.</summary>
    Public Property AllowedGames As Config_App.Game_Enum() = Nothing

    ''' <summary>Carpeta de las caches del diccionario de Data.</summary>
    Public Property CacheDirectory As String = IO.Path.Combine(Application.StartupPath, "Caches")

    ''' <summary>Firmas a leer de los plugins. Nothing = el filtro por defecto del lector (el de NPC Manager).</summary>
    Public Property SignatureFilter As HashSet(Of String) = Nothing

    ''' <summary>Idioma de los STRINGS forzado por la app (p. ej. -l:). Nothing = el del Fallout4.ini.</summary>
    Public Property LanguageOverride As String = Nothing

    ''' <summary>False = la app no guarda la seleccion: se oculta la casilla «Remember».</summary>
    Public Property RememberSelectionSupported As Boolean = True

    ' ============================================================================================== hooks (defaults neutros)

    ''' <summary>La ultima seleccion confirmada para ese juego (su orden literal), o vacio.</summary>
    Protected Overridable Function LoadSavedSelection(game As Config_App.Game_Enum) As IEnumerable(Of String)
        Return Array.Empty(Of String)()
    End Function

    ''' <summary>Guarda la seleccion (orden literal de la grilla) o la BORRA con Nothing (el usuario destildo «Remember»).</summary>
    Protected Overridable Sub SaveSelection(game As Config_App.Game_Enum, literalOrNothing As List(Of String))
    End Sub

    ''' <summary>Persiste las rutas (exe, overrides de Plugins.txt e INIs) que viven en Config_App.</summary>
    Protected Overridable Sub SavePathsConfig()
        Config_App.SaveConfig()
    End Sub

    ''' <summary>El juego queda FIJADO (antes de cargar): la app aplica lo que depende del juego.</summary>
    Protected Overridable Sub OnGameCommitted(game As Config_App.Game_Enum)
    End Sub

    ''' <summary>Pasos propios de la app despues de cargar plugins y montar Data, dentro del mismo Try (un error cae en el
    ''' mismo «Load failed»). setStatus escribe la linea de progreso.</summary>
    Protected Overridable Function RunPostLoadStepsAsync(pm As PluginManager, dataPath As String,
                                                        selected As IReadOnlyList(Of String),
                                                        setStatus As Action(Of String)) As Task
        Return Task.CompletedTask
    End Function

    Private ReadOnly Property SingleGame As Boolean
        Get
            Return AllowedGames IsNot Nothing AndAlso AllowedGames.Length = 1
        End Get
    End Property
    ...
    Private Sub Preflight_Form_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Me.Text = WindowTitle
        CheckBoxPersistSelection.Visible = RememberSelectionSupported
        WireOrderTooltips()
        If SingleGame Then Config_App.Current.Game = AllowedGames(0)
        ...
        ComboBoxGame.Enabled = Not SingleGame
    ...
    Private Sub RefreshPluginList()
        ...
        ' LoadSavedSelection; in NPC Manager NPC_Config.PreflightSelection_FO4/_SSE). Filtered against what's actually in Data\ — a plugin
        ...
        Dim savedSelection = LoadSavedSelection(Config_App.Current.Game).
...
#End Region
    ...
    ''' confirmed for this game (LoadSavedSelection), so "actives" is no longer necessarily
    ...
    Private Async Sub ButtonOk_Click(sender As Object, e As EventArgs) Handles ButtonOk.Click
        ...
        OnGameCommitted(Config_App.Current.Game)
        ...
        ' the actives default.
        SaveSelection(Config_App.Current.Game,
                      If(RememberSelectionSupported AndAlso CheckBoxPersistSelection.Checked, literalSelection, Nothing))
        ...
        Try
            ...
            Await Task.Run(Sub() pm.LoadAllPlugins(LoadedDataPath, SelectedPlugins, pluginProgress, SignatureFilter))
            ...
            ' Pasos propios de la app (NPC Manager: plugins autogenerados y sidecars .bssliders).
            Await RunPostLoadStepsAsync(pm, LoadedDataPath, SelectedPlugins, Sub(msg) LabelProgress.Text = msg)
