...
Friend Module ParserHelpers
    ...
    ''' <para>Sin este corte el valor se remapea: el byte alto <c>0xFF</c> nunca es un índice de master
    ''' válido (el tope es 0xFD full / 0xFE light), así que cae en la rama "self", se le toma el object id
    ''' 0xFFFFFF y sale como un FormID del propio archivo. MEDIDO: el peor ejemplo de
    ...
    ''' PLAUSIBLE Y EQUIVOCADO, sin error. Por eso <paramref name="pluginManager"/> NO es
    ''' <c>Optional</c> en estos parsers: una firma opcional anuncia como soportado un modo que corrompe.
    ''' Esta guarda queda como red para quien pase Nothing a propósito — un acto explícito, no un
