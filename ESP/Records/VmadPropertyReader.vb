...
'''
''' Distinto de un escaneo de POSICIONES de FormID, que anota offsets de byte para que el
''' writer can remap them and never interprets the payload). Here we want typed values keyed by property name.
'''
''' LAYOUT of the VMAD blob (IDENTICAL shape in both games, only the property-type set differs):
...
Friend NotInheritable Class VmadPropertyReader
    ...
    ''' <summary>u16-length-prefixed string, advancing the cursor.</summary>
    ...
    ''' <summary>FO4 Struct: u32 member count, then memberName/Type/Flags/Value each.</summary>
