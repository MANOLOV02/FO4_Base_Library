...
'
' HECHOS (verificados contra los datos reales del juego):
...
Public NotInheritable Class RaceBehaviorResolver
    ...
    ' Bit 0 de los flags de template = "Use Traits" (incluye Race). Ver MainForm TraitsState.
    Private Const TemplateFlagUseTraits As UShort = &H1US

    ' Número de función de condición GetIsRace: su primer parámetro es el RACE al que la condición se
    ' refiere. Es lo único que se mira de las CTDA de un IDLE.
    Private Const FuncionGetIsRace As UShort = 69US

    ' ── KYWD.TNAM Type — discriminador AUTORITATIVO de los SAKD.
    ...
    ' ⛔ Guarda las VISTAS, no una copia de sus campos. Cada vista sostiene el árbol del record del que
    ' salió: soltarla y quedarse con el nodo, o al revés, deja la mitad viva y la otra colgada. El árbol se
    ' arma UNA vez por IDLE y desde ahí se lee tantas veces como razas haya.
    Private Shared _parsedIdles As List(Of Canon.IdleFO4)              ' TODOS los IDLE leídos UNA vez (tabla global, race-independiente)
    ...
    Public Shared Sub EnsureKeywordMaps(pm As PluginManager)
        ...
        For Each rec In pm.GetRecordsOfType("RACE")
            ...
            For Each k In race.Keywords
                ...
                If kt.TryGetValue(k.Keyword, tt) AndAlso tt = KwTypeNone Then ident.Add(k.Keyword)   ' None-typed ∧ declarada por una raza = identidad
            Next
        Next
        ' Lectura de TODOS los records IDLE UNA sola vez (la tabla IDLE es global, no por-raza). ResolveRaceIdles solo
        ' FILTRA esta lista por condición de raza — evita re-leer ~3691 IDLE (CTDA) en cada render.
        ...
        Dim idles As New List(Of Canon.IdleFO4)
        If Config_App.Current.Game = Config_App.Game_Enum.Fallout4 Then
            For Each rec In pm.GetRecordsOfType("IDLE")
                Dim idle As Canon.IdleFO4 = Nothing
                Try : idle = TryCast(Canon.CanonRecords.Idle(rec, pm), Canon.IdleFO4) : Catch : Continue For : End Try
    ...
    Public Shared Function ResolveNpcBehavior(npc As NPC_Data, pm As PluginManager) As ResolvedRaceBehavior
        If IsNothing(npc) OrElse IsNothing(npc.Record) OrElse IsNothing(pm) Then Return Nothing
        Dim raceFid = ResolveEffectiveRaceFormID(npc.Record, pm, New HashSet(Of UInteger)())
        Dim result = ResolveRaceBehavior(raceFid, pm)
        If result IsNot Nothing Then result.IsFemale = npc.Record.ConfigurationFlagsFemale
        Return result
    End Function

    ''' <summary>Raza efectiva: con Use Traits + template, sigue la cadena TPLT; si no, la RNAM propia.</summary>
    Private Shared Function ResolveEffectiveRaceFormID(npc As Canon.INpc, pm As PluginManager, visited As HashSet(Of UInteger)) As UInteger
        If IsNothing(npc) Then Return 0UI
        Dim plantilla = npc.Plantilla()
        If (npc.ConfigurationTemplateFlags And TemplateFlagUseTraits) <> 0US AndAlso plantilla <> 0UI AndAlso visited.Add(plantilla) Then
            Dim tmplRec = pm.GetRecord(plantilla)
            If tmplRec IsNot Nothing AndAlso tmplRec.Header.Signature = "NPC_" Then
                Dim tmpl = Canon.CanonRecords.Npc(tmplRec, pm)
    ...
    Public Shared Function ResolveRaceBehavior(raceFormID As UInteger, pm As PluginManager) As ResolvedRaceBehavior
        ...
        ' Cache por raza: leer la RACE (hasta 2601 subgraphs vía SRAC) + filtro IDLE se hacen UNA vez por raza, no por render.
        ...
        Dim result As New ResolvedRaceBehavior With {
            ...
            .MaleProject = race.MaleBehaviorGraphModelFileName,
            .FemaleProject = race.FemaleBehaviorGraphModelFileName,
            .MaleSkeleton = race.MaleSkeletalModel,
            .FemaleSkeleton = race.FemaleSkeletalModel
        }
        ' Keywords del race EFECTIVO (el 'Anims<X>Race' que filtra los subgraphs compartidos por SAKD).
        result.ActorKeywords.AddRange(race.Keywords.Select(Function(k) k.Keyword))

        ' Subgraph Data / SRAC / SADD son exclusivos de Fallout 4 — Skyrim no los declara en RACE.
        If raceFo4 IsNot Nothing Then
            Dim subgraphs = raceFo4.SubgraphData
            If subgraphs.Count > 0 Then
                result.Subgraphs.AddRange(subgraphs)
                result.SubgraphSource = "own"
            ElseIf raceFo4.SubgraphTemplateRace <> 0UI Then
                result.Subgraphs.AddRange(ResolveRaceSubgraphs(raceFo4.SubgraphTemplateRace, pm, New HashSet(Of UInteger)()))
                result.SubgraphSource = "SRAC:0x" & raceFo4.SubgraphTemplateRace.ToString("X8")
            End If
            If raceFo4.SubgraphAdditiveRace <> 0UI Then
                result.Subgraphs.AddRange(ResolveRaceSubgraphs(raceFo4.SubgraphAdditiveRace, pm, New HashSet(Of UInteger)()))
                result.SubgraphSource = (result.SubgraphSource & " +SADD:0x" & raceFo4.SubgraphAdditiveRace.ToString("X8")).Trim()
            End If
        End If
    ...
    Private Shared Sub ResolveRaceIdles(rb As ResolvedRaceBehavior)
        ...
        For Each idle In idles   ' leídos 1×/pm en EnsureKeywordMaps — acá SOLO se filtra por raza
            ' GetIsRace: si hay condiciones positivas, alguna debe ser esta raza; si hay una negativa para esta raza, excluir.
            Dim hayPositiva As Boolean = False
            Dim positivaDeEstaRaza As Boolean = False
            Dim excluida As Boolean = False
            For Each cond In idle.Conditions
                If cond.ConditionFunction <> FuncionGetIsRace Then Continue For
                ' "== 1" → aplica a esa raza; "== 0" → la EXCLUYE. El operador vive en los bits altos del
                ' tipo y NotEqual (1) invierte el sentido.
                Dim positiva As Boolean = (cond.ConditionComparisonValueFloat >= 0.5F)
                If (cond.ConditionType >> 5) = 1 Then positiva = Not positiva
                If positiva Then
                    hayPositiva = True
                    If cond.Parameter1Race = rb.RaceFormID Then positivaDeEstaRaza = True
                ElseIf cond.Parameter1Race = rb.RaceFormID Then
                    excluida = True
                End If
            Next
            If excluida Then Continue For
            If hayPositiva AndAlso Not positivaDeEstaRaza Then Continue For
    ...
    Private Shared Function ResolveRaceSubgraphs(raceFormID As UInteger, pm As PluginManager, visited As HashSet(Of UInteger)) As List(Of Canon.RaceFO4_SubgraphData)
        Dim r As New List(Of Canon.RaceFO4_SubgraphData)
        ...
        Dim raceFo4 = TryCast(Canon.CanonRecords.Race(rec, pm), Canon.RaceFO4)
        If IsNothing(raceFo4) Then Return r
        Dim subgraphs = raceFo4.SubgraphData
        If subgraphs.Count > 0 Then
            r.AddRange(subgraphs)
        ElseIf raceFo4.SubgraphTemplateRace <> 0UI Then
            r.AddRange(ResolveRaceSubgraphs(raceFo4.SubgraphTemplateRace, pm, visited))
        End If
        Return r
    End Function
End Class
...
Public Class ResolvedRaceBehavior
    ...
    Public ReadOnly Property Subgraphs As New List(Of Canon.RaceFO4_SubgraphData)
    ...
    Public Function DistinctBehaviorFiles() As List(Of String)
        ...
        all.AddRange(Subgraphs.Select(Function(s) s.DataBehaviourGraph))
        For Each p In all
